export const EBM_ACCENT = '#1e446f';

export function Accent({ children }: { children: React.ReactNode }) {
  return (
    <span className="font-semibold" style={{ color: EBM_ACCENT }}>
      {children}
    </span>
  );
}

export function EbmCaseSection({
  label,
  title,
  children,
  reverse,
          ...
          <div className="min-w-0 flex-1 space-y-4">
            {showHeader &&
              (variant === 'default' ? (
                ...
                title && (
                  <h2 className={`border-l-4 pl-4 ${titleClass}`} style={{ borderColor: EBM_ACCENT }}>
                    {title}
                  </h2>
                )
              ))}
