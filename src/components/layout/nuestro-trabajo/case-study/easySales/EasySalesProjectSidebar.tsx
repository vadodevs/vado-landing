...
import { EASY_SALES_ACCENT } from './easySales-case-section';
...
export function EasySalesProjectSidebar() {
    ...
    <div className="rounded-2xl border border-slate-200 bg-white shadow-md">
      <div className="border-border border-b px-5 py-4">
        <h3 className="flex items-center gap-2 text-sm font-bold tracking-wider text-slate-700 uppercase">
          <FaClipboardList className="size-4 shrink-0" style={{ color: EASY_SALES_ACCENT }} />
          {t('ourWork.caseStudy.easySales.sidebar.title')}
        </h3>
      </div>
      ...
      <div className="divide-y divide-slate-100 px-5 py-4">
        <dl className="space-y-4 text-sm">
          <div>
            <dt className="text-muted-foreground mb-1 flex items-center gap-2 text-xs font-semibold tracking-wider uppercase">
              <FaIndustry className="size-3.5 shrink-0" style={{ color: EASY_SALES_ACCENT }} />
              {t('ourWork.heroBadge.industry')}
            </dt>
            <dd className="font-medium text-slate-700">
              {t('ourWork.caseStudy.easySales.hero.industry')}
            </dd>
          </div>
          <div className="pt-3">
            <dt className="text-muted-foreground mb-1 flex items-center gap-2 text-xs font-semibold tracking-wider uppercase">
              <FaLaptop className="size-3.5 shrink-0" style={{ color: EASY_SALES_ACCENT }} />
        ...
        <div className="pt-4">
          <p className="text-muted-foreground mb-3 flex items-center gap-2 text-xs font-semibold tracking-wider uppercase">
            <FaCode className="size-3.5 shrink-0" style={{ color: EASY_SALES_ACCENT }} />
            {t('ourWork.caseStudy.easySales.stackLabel')}
          </p>
          <ul className="flex flex-wrap gap-2" role="list">
            {EASY_SALES_STACK.map((item) => (
              <li key={item.icon}>
                <span
