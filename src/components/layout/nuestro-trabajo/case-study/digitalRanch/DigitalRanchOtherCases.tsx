...
export function DigitalRanchOtherCases() {
  const { t } = useTranslation();
  const { path } = useLocale();
  const otherProjects = OUR_WORK_PROJECTS.filter((p) => p.id !== 'digitalRanch').slice(0, 3);
    ...
    <section className="border-border bg-muted/30 border-t py-16 md:py-24">
      <CenterContainer>
        <h2 className="mb-10 text-2xl font-bold tracking-tight text-slate-700 md:text-3xl">
          {t('ourWork.caseStudy.digitalRanch.otherCases.title')}
        </h2>
        <motion.ul
