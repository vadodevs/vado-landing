export const SENDERO_ACCENT = '#27bfad';

export function Accent({ children }: { children: React.ReactNode }) {
  return (
    <span className="font-semibold" style={{ color: SENDERO_ACCENT }}>
      {children}
    </span>
  );
}

export function SenderoCaseSection({
  label,
  title,
  children,
  reverse,
          ...
          <div className="min-w-0 flex-1 space-y-4">
            {showHeader &&
              (variant === 'default' ? (
                ...
                title && (
                  <h2 className={`border-l-4 pl-4 ${titleClass}`} style={{ borderColor: SENDERO_ACCENT }}>
                    {title}
                  </h2>
                )
              ))}
